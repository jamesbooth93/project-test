...
def scenario_one_read_level(history):
    weekly_best = {}
    for snapshot in history or []:
        ...
        if "strong" in qualities:
            weekly_best[week] = "strong"
        elif "acceptable" in qualities:
            weekly_best[week] = "acceptable"
        elif "weak" in qualities or not actions:
            weekly_best[week] = "weak"

    strong_total = sum(1 for quality in weekly_best.values() if quality == "strong")
    strong_early = sum(1 for week, quality in weekly_best.items() if week and week <= 3 and quality == "strong")
    acceptable_total = sum(1 for quality in weekly_best.values() if quality in {"strong", "acceptable"})

    if strong_early >= 2 and strong_total >= 4:
        return "early_read"
    if strong_total >= 2 and acceptable_total >= 4:
        return "late_shift"
    if acceptable_total >= 2:
        return "visible_only"
    return "blind"
