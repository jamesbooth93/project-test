...
def resolve_intervention_outcome(game, node, action_type, fit, secondary_node=None):
    ...
    authored_quality = scenario_runtime.authored_action_quality(
        game,
        action_type,
        node=node,
        secondary_node=secondary_node,
    )
    if authored_quality == "strong":
        success_prob = float(np.clip(max(success_prob, 0.70) + 0.10, 0.08, 0.95))
        backfire_prob = float(np.clip(min(backfire_prob, 0.05) - 0.01, 0.01, 0.55))
    elif authored_quality == "acceptable":
        success_prob = float(np.clip(max(success_prob, 0.52) + 0.04, 0.08, 0.90))
        backfire_prob = float(np.clip(backfire_prob - 0.02, 0.01, 0.55))
    elif authored_quality == "weak":
        success_prob = float(np.clip(success_prob - 0.08, 0.08, 0.88))
        backfire_prob = float(np.clip(backfire_prob + 0.04, 0.02, 0.55))
    ...
    if roll < success_prob:
        ...
        outcome = "failure"
    if authored_quality == "strong" and outcome == "failure":
        outcome = "partial"
    return {
        "outcome": outcome,
        "success_prob": float(success_prob),
