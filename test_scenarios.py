...
class ScenarioRegressionTests(unittest.TestCase):
    ...
    def test_scenario_01_week_one_strong_opening_bends_raw_cluster_strain_downward(self):
        ...
        no_action_game.end_week()
        no_action_snapshot = no_action_game.get_analysis_history()[-1]

        strong_game = GameState(
            scenario="scenario_01",
            difficulty="Normal",
            team_size=SCENARIOS["scenario_01"].team_size,
            max_weeks=SCENARIOS["scenario_01"].length,
        ...
        focal = strong_game.get_scenario_role_node_id("focal_employee")
        strong_game.apply_player_action({"type": "quick_check_in", "target": focal})
        strong_game.apply_player_action({"type": "clarify_roles_and_handoffs", "target": focal})
        strong_game.end_week()
        strong_snapshot = strong_game.get_analysis_history()[-1]

        def cluster_avg(snapshot):
        ...
        self.assertLess(cluster_avg(strong_snapshot), cluster_avg(no_action_snapshot))
