...
def render_evidence(vm, game):
    ...
    with lower_left:
        ...
        dependency_note = ""
        if game.scenario == "scenario_02":
            if selected_row.get("scenario_role") == "hidden_strain_employee":
                dependency_note += (
                    "<div class=\"employee-note\"><strong>Working style:</strong> "
                    "Helpful, steady, and often the person others turn to when work gets messy.</div>"
                )
        st.markdown(
            f"""
            <div class="employee-panel">
                ...
                <div class="employee-note"><strong>{'Visible friction' if game.scenario == 'scenario_02' else 'Visible concern'}:</strong> {selected_row['risk_label']}</div>
                <div class="employee-note"><strong>Risks noticed:</strong> {format_warning_signs(selected_row['warning_signs'])}</div>
                <div class="employee-note"><strong>Current engagement:</strong> {selected_row['engagement_hint']}</div>
                <div class="employee-note"><strong>Pressure clue:</strong> {selected_row.get('pressure_clue', 'No clear pressure clue yet')}</div>
                <div class="employee-note"><strong>Background load:</strong> {visible_load_label(float(selected_row.get('absorbed_workload_value', 0.0)))} - {selected_row.get('workload_clue', '')}</div>
                <div class="employee-note"><strong>Coordination pull:</strong> {selected_row.get('dependency_clue', 'No clear coordination clue yet')}</div>
                {dependency_note}
            </div>
            """,
            unsafe_allow_html=True,
...
def render_decision_space(vm, game):
    ...
    action_options = {
        f"{action_description(item['key'])} · {int(item['cost'])} energy": item["key"]
...
def render_final_score(game):
    ...
    benchmark_avg = average_cluster_strain_across_history(benchmark_history)

    st.markdown('<div class="shell-card">', unsafe_allow_html=True)
    st.markdown('<h2 class="centered-outcome">Final Score</h2>', unsafe_allow_html=True)
    if player_avg is not None:
        st.markdown(
            f'<div class="summary-impact">Average core-group strain across the run: {round(player_avg * 100)}%</div>',
            unsafe_allow_html=True,
        )
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    if player_avg is not None and target_avg is not None:
        if player_avg <= target_avg + 0.005:
            copy = (
                f'You finished with an average core-group strain of {round(player_avg * 100)}%. '
...
def render_end_screen(game):
    ...
    if results_view == "summary":
        ...
        if player_high_strain > benchmark_high_strain:
            ...
            st.markdown('</div>', unsafe_allow_html=True)
        elif latest and benchmark_latest:
            end_comparison = cluster_strain_improvement(latest, benchmark_latest)
    ...
    st.markdown('<div class="shell-card">', unsafe_allow_html=True)
    st.markdown(f'<h2 class="centered-outcome">Analysis</h2>', unsafe_allow_html=True)
    st.markdown(f'<div class="centered-outcome">{summary_title}</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    if nav_left.button("Previous Week", width="stretch", disabled=current_index == 0):
        if current_index > 0:
            ...
            st.rerun()
    nav_mid.markdown(
        f'<div class="centered-results-text"><strong>{analysis_step_label(selected_week)}</strong></div>',
        unsafe_allow_html=True,
    )
